...

{
...

		{
...

			{
				...
				case 'quantity':
					...
					$sql_query['select'] = 'SELECT p.`id_product`, product_shop.`condition`, p.`id_manufacturer`, sa.quantity as `quantity`, p.`weight` ';
				...
				case 'id_attribute_group':// attribute group
					...
					AND p.id_product IN (
						...
						LEFT JOIN '._DB_PREFIX_.'category c ON (c.id_category = cp.id_category AND
...

	{
        ...
        $query_filters_join = 'INNER JOIN `'._DB_PREFIX_.'egms_delivery sav ON (sav.id_manufacturer = p.id_manufacturer AND sav.id_egms_cu = '.(int)Shop::getUtlId().') ';
