...
class Category extends CategoryCore{
	...
	public function __construct($id_category = null, $id_lang = null, $id_shop = null, $complement = '')
	{
        ...
        $c = str_replace('https://' , '' , str_replace('http://', '', $this->getLink()));
        $url = Tools::getHttpHost() . $_SERVER['REQUEST_URI'];
        //  if(trim($url) != '')
        // {
            $url = strtok($url, '?');
            $url = strtok($url, '#');
        ...
        $filter = egmsfilter::getFilterData($url, $this->id);
         //$complement = " > conplement";
        if ($filter['id_filter']!='')
        {
            $this->meta_title = ($filter['meta_title']=='')?Meta::replaceForCEOWord($this->meta_title).$complement:$filter['meta_title'];
            $this->meta_keywords = ($filter['meta_keywords'==''])?Meta::replaceForCEOWord($this->meta_keywords).$complement:$filter['meta_keywords'==''];
            $this->meta_description = ($filter['meta_description']=='')?Meta::replaceForCEOWord($this->meta_description).$complement:$filter['meta_description'];
            $this->name = ($filter['title']=='')?Meta::replaceForCEOWord($this->name).$complement:$filter['title'];
            $this->description = $filter['description'];
        }else{
            $this->meta_title = Meta::replaceForCEOWord($this->meta_title).$complement;
            $this->meta_keywords = Meta::replaceForCEOWord($this->meta_keywords).$complement;
            $this->meta_description = Meta::replaceForCEOWord($this->meta_description).$complement;
            $this->name = Meta::replaceForCEOWord($this->name).$complement;
            if ($url!='')
                $this->description = '';
            else
