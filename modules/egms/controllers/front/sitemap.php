...
class egmssitemapModuleFrontController extends ModuleFrontController
{
	public function initContent()
	{
		...
		$page = $this->getSitemap(Tools::getValue('map_type'), Tools::getValue('map_city'));
		
		$this->context->smarty->assign(array(
			'page' => $page
		));	
		
		$this->smartyOutputContent($this->getTemplatePath('page.tpl'));
		
	}


    public function getSitemap($map_type, $map_city="")
    {
        $citys = 1;// Configuration::get('EGMS_CITYS');
        $res = "";
        if ($map_type)
        {
            if($map_city)
