...
class BlockLayered extends Module
{
	...
	public function __construct()
	{
        ...
        // $filter = egmsfilter::getFilterData($filter_block['current_friendly_url']);
	}
	...
	public function hookProductListAssign($params)
	{
        ...
        $this->category_count = $this->nbr_products;
        $this->category_description = $this->category->description;
        $this->replaceCEO();

        $smarty->assign('meta_title', $this->meta_title);
        $smarty->assign('meta_description', $this->meta_description);
        $smarty->assign('meta_keywords', $this->meta_keywords);

		$smarty->assign('filter_category_name', $this->category_title);
        $smarty->assign('categoryNameComplement', $title);
        $smarty->assign('filter_category_description', $this->category_description);


		// Need a nofollow on the pagination links?
		//$smarty->assign('no_follow', $filter_block['no_follow']);
       //EG $smarty->assign('no_follow', $filter_block['no_follow']);
	}
	...
	public function categoryRedirect($id_category)
    {
        $row = egmsfilter::getRedirectUrl($id_category);
        if($row)
            return $row;
        else
            return false;
	...
	public function hookHeader($params)
    {
        ...
        $redirect = $this->categoryRedirect($filter_block['id_category_layered']);
        if($redirect){
            $link = new Link();
            $url = $link->getCategoryLink($redirect['id_category']).$redirect['url'];
            $this->sslRedirection($url);
        }


        $this->current_url = $filter_block['current_friendly_url'];//$_SERVER['REQUEST_URI'];
        $this->filter = egmsfilter::getFilterData($this->current_url, $filter_block['id_category_layered']);
        ...
        $this->context->selected_filter = $filter_block['selected_filters'];
        ...
        /*
                if (is_array($filter_block['title_values']))
                    foreach ($filter_block['title_values'] as $key => $val)
                    {
                       $title_add .= ' > '.$key.' '.implode('/', $val);
                        $keywords .= $key.' '.implode('/', $val).', ';
                        $filter_block['url_path'][1][] = $key.' '.implode('/', $val);
                    }*/

        if (is_array($filter_block['meta_values']))
            foreach ($filter_block['meta_values'] as $val) {
                $title_add .= ' > ' . $val['title'] . ' ' . implode(' / ', $val['values']);;
                $keywords .= $val['title'] . ' ' . implode('/', $val['values']) . ', ';
                $filter_block['url_path'][1][] = $val['title'] . ' ' . implode(' / ', $val['values']);
            }


        $category = new Category((int)Tools::getValue('id_category'), $id_lang, null, $title_add);
        ...
        $this->context->selected_filter_bread = $filter_block['url_path'];
        if($this->filter['bread']!=''){
            $this->context->selected_filter_bread[0][] = $this->filter['url'];
            $this->context->selected_filter_bread[1][] = $this->filter['bread'];
        }
        ...
        if (!empty($title))
            ...
            $this->meta_title = $category_metas['meta_title'];


        if ($category->meta_description == $category_metas['meta_description'])
            $this->meta_description = $category_metas['meta_description']; //$category_metas['meta_description'];
        else
            $this->meta_description = $category->meta_description;

        $keywords = substr(strtolower($keywords), 0, 1000);
        $this->meta_keywords = rtrim($category_title . ', ' . $keywords . ', ' . $category_metas['meta_keywords'], ', ');

        $smarty->assign('meta_title', $this->meta_title);
        $smarty->assign('meta_description', $this->meta_description);
        $smarty->assign('meta_keywords', $this->meta_keywords);

        $subcats = egmsfilter::getSubCategorys($this->filter['id_filter'], $category->getLink());
        if($subcats)
        ...
        $filter_nofolow = true;
        $filter_norobots = true;

        if($this->isCategoryLink($category->getLink())) {
            $filter_nofolow = false;
            $filter_norobots = false;
        }else {
        ...
        if($filter_norobots)
            $smarty->assign('nobots', $filter_norobots);
        $smarty->assign('nofollow', $filter_nofolow);

        if($this->filter['url']!='')
            $smarty->assign('canonical', rtrim($category->getLink().$this->filter['url'],'/'));
        else
            $smarty->assign('canonical', $category->getLink().$filter_block['current_friendly_url']);
...
*/
	...
	public function isCategoryLink($category_link){

        $url = Tools::getProtocol(Tools::usingSecureMode()).$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'];
    ...
    protected function sslRedirection($url)
    {
			...
			if (!empty($url_attributes))
			{
				foreach ($url_attributes as $url_attribute)
				{
					...
					else
					{
						...
						{
                            $url_key = md5('/'.$attribute_name.$this->getAnchor().$url_parameter);
                            $cache_id = 'BlockLayered::getSelectedFilters'.$url_key;

                            if (!Cache::isStored($cache_id)) {
                                $data = Db::getInstance()->getValue('SELECT data FROM `' . _DB_PREFIX_ . 'layered_friendly_url` WHERE `url_key` = \'' . $url_key . '\'');
		...
		foreach ($selected_filters as $key => $filter_values)
		{
			...
			switch ($key)
			{
				...
				case 'quantity':
                    ...
                    //$query_filters_where .= ' and sa.quantity = 1 ';
                    $query_filters_from .= ' LEFT JOIN `'._DB_PREFIX_.'egms_delivery` sa ON (sa.id_manufacturer = p.id_manufacturer AND sa.id_egms_cu = '.(int)Shop::getUtlId().') ';
				break;
			...
			if (version_compare(_PS_VERSION_, '1.6.1', '>=') === true)
			{
				...
				SELECT
					...
					'.(Combination::isFeatureActive() ? 'product_attribute_shop.id_product_attribute id_product_attribute,' : '').'
					 DATEDIFF('.$alias_where.'.`date_add`, DATE_SUB("'.date('Y-m-d').' 00:00:00", INTERVAL '.(int)$nb_day_new_product.' DAY)) > 0 as new, 0 as out_of_stock, 0 as quantity'.(Combination::isFeatureActive() ? ', product_attribute_shop.minimal_quantity AS product_attribute_minimal_quantity' : '').'
				FROM (select * FROM '._DB_PREFIX_.'cat_filter_restriction cp
				...
				' LIMIT '.(((int)$this->page - 1) * $n.','.$n).') cp1
				LEFT JOIN `'._DB_PREFIX_.'product` p ON p.`id_product` = cp1.`id_product`
			...
			else
			{
				...
				SELECT
					...
					'.(Combination::isFeatureActive() ? 'MAX(product_attribute_shop.id_product_attribute) id_product_attribute,' : '').'
					 DATEDIFF('.$alias_where.'.`date_add`, DATE_SUB("'.date('Y-m-d').' 00:00:00", INTERVAL '.(int)$nb_day_new_product.' DAY)) > 0 as new, 0 as out_of_stock, 0 as quantity'.(Combination::isFeatureActive() ? ', MAX(product_attribute_shop.minimal_quantity) AS product_attribute_minimal_quantity' : '').'
				FROM (select cp.* FROM '._DB_PREFIX_.'cat_filter_restriction cp 
				INNER JOIN `'._DB_PREFIX_.'product` p ON p.`id_product` = cp.`id_product`
				'.Shop::addSqlAssociation('product', 'p').'
				WHERE '.$alias_where.'.`active` = 1 AND '.$alias_where.'.`visibility` IN ("both", "catalog")
				 ORDER BY p.rprice, p.price asc
				-- ORDER BY '.Tools::getProductsOrder('by', Tools::getValue('orderby'), true).' '.Tools::getProductsOrder('way', Tools::getValue('orderway')).' , cp.id_product
'.
                    ' LIMIT '.(((int)$this->page - 1) * $n.','.$n).') cp1
                    INNER JOIN `'._DB_PREFIX_.'product` p ON p.`id_product` = cp1.`id_product`
				'.Shop::addSqlAssociation('product', 'p').
                    (Combination::isFeatureActive() ?
				...
				'
                    ;
				$this->products = Db::getInstance()->executeS($s, true, false);
			}
		}
    ...
    public function hookDisplayProductListLayered($params){

        $id_product = (int)$params['product']['id_product'];

        if (!$this->isCached('product-list.tpl', $this->getCacheId($id_product)))
        {
            $params['product']['link'] = $this->context->link->getProductLink($params['product']['id_product'], null, null, null, null, null, $params['product']['id_product_attribute'], Configuration::get('PS_REWRITING_SETTINGS'), false, true);
            $params['product']['image_link'] = $this->context->link->getImageLink($params['product']['link_rewrite'], $params['product']['id_image'], 'home_default');
        ...
        return $this->display(__FILE__, 'product-list.tpl', $this->getCacheId($id_product));
    }
    ...
    /*    public function hookDisplayProductDeliveryTime($params){
            ...
            if (!$this->isCached('product-list-attribute.tpl', $this->getCacheId($id_product, $id_product_attribute)))
            {
                $s = 'select * from ps_attribute_lang where id_attribute !='.$id_product;
                $a = Db::getInstance()->executeS($s);
                $this->smarty->assign(array(
                    'my_product' => $a,
                    'pa' => $id_product_attribute,
                ));
            }
            return $this->display(__FILE__, 'product-list-attribute.tpl', $this->getCacheId($id_product, $id_product_attribute));
    }*/
	...
	public function getFilterBlock($selected_filters = array())
	{
		...
		$this->filtes_index_array = egmsfilter::getIndexFilters($id_parent);

		/* Get the filters for the current category */
		$filters = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			SELECT type, id_value, filter_show_limit, filter_type, show_on_form, position, position_url FROM '._DB_PREFIX_.'layered_category
			WHERE id_category = '.(int)$id_parent.'
				AND id_shop = '.$id_shop.'
			GROUP BY `type`, id_value ORDER BY position_url ASC'
		);
		...
		Db::getInstance()->execute('CREATE TEMPORARY TABLE '._DB_PREFIX_.'cat_restriction ENGINE=MEMORY
													...
													AND p.id_manufacturer '. egms_shop::getManufacturerByShop('in'), false);
		...
		foreach ($filters as $filter)
		{
			...
			switch ($filter['type'])
			{
				case 'price':
					...
					$sql_query['where'] .= ' AND p.id_manufacturer ' . egms_shop::getManufacturerByShop('in') ;
					break;
				case 'weight':
				...
				case 'condition':
					...
					break;
				case 'quantity':
					$sql_query['select'] = 'SELECT p.`id_product`, sa.`quantity`, 0 as `out_of_stock` ';
                    ...
                    $sql_query['join'] .= 'LEFT JOIN `'._DB_PREFIX_.'egms_delivery` sa
						ON (sa.id_manufacturer = p.id_manufacturer AND sa.id_egms_cu = '.(int)Shop::getUtlId().') ';
					$sql_query['where'] = 'WHERE 1';
					break;

				case 'manufacturer':
					$sql_query['select'] = 'SELECT /* COUNT(DISTINCT p.id_product) */ 1 nbr, m.id_manufacturer, m.name ';
					$sql_query['from'] = '
					FROM '._DB_PREFIX_.'cat_restriction p
				...
				case 'id_attribute_group':// attribute group
					$sql_query['select'] = '
					SELECT /* COUNT(DISTINCT lpa.id_product) */ 1 nbr, lpa.id_attribute_group,
					...
					$sql_query['from'] = '
					FROM(select lpa1.id_attribute_group, lpa1.id_attribute, lpa1.id_product FROM '._DB_PREFIX_.'layered_product_attribute lpa1
					WHERE lpa1.id_attribute_group = '.(int)$filter['id_value'].'
					AND lpa1.`id_shop` = '.(int)$context->shop->id.'
                    ) lpa	
				...
				case 'id_feature':
					...
					/* COUNT(DISTINCT p.id_product) */ 1 nbr,
			...
			switch ($filter['type'])
			{
				...
				if ($this->showPriceFilter()) {
					$price_array = array(
                        ...
                        'show_on_form' => $filter['show_on_form'],
		...
		foreach ($filter_blocks as &$type_filter)
		{
			$filter_name1 = (!empty($type_filter['url_name']) ? $type_filter['url_name'] : $type_filter['name']);
			$filter_link_rewrite = Tools::link_rewrite($filter_name1);
			...
			if (count($type_filter) > 0 && !isset($type_filter['slider']))
			{
				foreach ($type_filter['values'] as $key => $values)
				{
                    ...
                    $link_part = '/'.ltrim($parameters, '/');
					$type_filter['values'][$key]['link'] = $categorie_link.$link_part;
					if(in_array($link_part, $this->filtes_index_array))
                        $nofollow = false;
		...
		$this->price_min = $price_array['min'];
		//$this->replaceCEO();

		foreach ($selected_filters as $filters)
			$n_filters += count($filters);

        usort($filter_blocks, function($a, $b){
      ...
      /*  $ll = array();
        foreach ($filter_blocks as $fb)
            foreach ($fb['values'] as $v)
                $ll[] = $v['link'];
*/
        $lt = 0;
		...
		$cache = array(
			...
			//EG 'no_follow' => (!empty($param_selected) || $global_nofollow)
		);

		return $cache;
	}

	public function replaceCEO(){

        $this->meta_title = str_replace('%price%', $this->price_min, $this->meta_title);
        $this->meta_description = str_replace('%price%', $this->price_min, $this->meta_description);
        $this->meta_keywords = str_replace('%price%', $this->price_min, $this->meta_keywords);
        $this->category_title = str_replace('%price%', $this->price_min, $this->category_title);
        $this->category_description = str_replace('%price%', $this->price_min, $this->category_description);

        $this->meta_title = str_replace('%count%', $this->category_count, $this->meta_title);
        $this->meta_description = str_replace('%count%', $this->category_count, $this->meta_description);
        $this->meta_keywords = str_replace('%count%', $this->category_count, $this->meta_keywords);
        $this->category_title = str_replace('%count%', $this->category_count, $this->category_title);
        $this->category_description = str_replace('%count%', $this->category_count, $this->category_description);
    }

	public static function getQueeFilter()
    {
	...
	private static function getQuantityFilterSubQuery($filter_value, $ignore_join = false)
	{
        ...
        //$query_filters = ' and sav.quantity = 1 ';
        $query_filters_join = 'LEFT JOIN `'._DB_PREFIX_.'egms_delivery` sav ON (sav.id_manufacturer = p.id_manufacturer AND sav.id_egms_cu = '.(int)Shop::getUtlId().') ';
	...
	public function ajaxCall()
	{
        ...
        $this->current_url = $filter_block['current_friendly_url'];//Tools::getProtocol(Tools::usingSecureMode()).$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'];
        $this->filter = egmsfilter::getFilterData($this->current_url, $filter_block['id_category_layered']);
        ...
        if (is_array($filter_block['meta_values']))
            foreach ($filter_block['meta_values'] as $val)
            {
                $title .= ' > '. $val['title'] . ' '.implode(' / ', $val['values']);;
                $keywords .= $val['title'].' '.implode(' / ', $val['values']).', ';
            }

        $category = new Category(Tools::getValue('id_category_layered', Configuration::get('PS_HOME_CATEGORY')), (int)$cookie->id_lang, null, $title);
...
//		$meta_description = $category_metas['meta_description'];
		...
		$this->meta_description = $category->meta_description;
		$this->meta_keywords = isset($meta_keywords) ? $meta_keywords : null;
		$this->category_title = $category->name;
		$this->category_count = $nb_products;
	...
	public function getProducts($selected_filters, &$products, &$nb_products, &$p, &$n, &$pages_nb, &$start, &$stop, &$range)
	{
        ...
        foreach ($key_values as $key => $key_value){
           ...
           // $n = Product::getAttributesParams($id_attributes[$key]['id']);
	...
	public function rebuildLayeredStructure()
	{
		...
		`show_on_form` int(10) UNSIGNED NOT NULL DEFAULT 1,
