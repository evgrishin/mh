...

	{
...

		{
			...
			SELECT
				...
				pl.*,
				MAX(i.`id_image`) id_image,
				il.legend,
				m.name manufacturer_name,
				MAX(pa.id_product_attribute) id_product_attribute,
                0 as new,
                0 as out_of_stock, 0 as quantity
			FROM `'._DB_PREFIX_.'product` p
			...
			LEFT JOIN '._DB_PREFIX_.'product_lang pl ON (pl.id_product = p.id_product AND pl.id_shop = product_shop.id_shop )
			
			LEFT JOIN '._DB_PREFIX_.'image i  ON (i.`id_product` = p.`id_product` and i.cover=1)
            LEFT JOIN '._DB_PREFIX_.'image_lang il ON (il.`id_image` = i.`id_image` AND il.`id_lang` = '.(int)$cookie->id_lang.')
            
            LEFT JOIN '._DB_PREFIX_.'product_attribute pa ON pa.id_product = p.id_product AND pa.default_on = 1
			
			WHERE p.id_product IN ('.implode(',', $product_id_list).')
