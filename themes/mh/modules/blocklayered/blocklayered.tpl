...
<div id="layered_block_left" class="block">
	<h4 class="title_block">{l s='Catalog1' mod='blocklayered'}</h4>
									...
									{if $filter.type == $filter_type && isset($filter.values)}
										...
										{else}
											{foreach from=$filter.values key=id_value item=value}
                                                ...
                                                {if isset($filter.show_on_form) && $filter.show_on_form > 0}
													<li>
								...
								{if $filter.filter_type == 0}
									{foreach from=$filter.values key=id_value item=value name=fe}
										{if $value.nbr || !$hide_0_values}
											<li class="nomargin {if $smarty.foreach.fe.index >= $filter.filter_show_limit}hiddable{/if} col-lg-{if ($filter.filter_show_limit==20)}6{else}12{/if}">
											...
											<label for="layered_{$filter.type_lite}_{$id_value}"{if !$value.nbr} class="disabled"{else}{if isset($filter.is_color_group) && $filter.is_color_group} name="layered_{$filter.type_lite}_{$id_value}" class="layered_color" data-rel="{$id_value}_{$filter.id_key}"{/if}{/if}>
												...
												{else}
												<a href="{$value.link}"{if $value.rel|trim != ''} rel="{$value.rel}" data-rel="{$value.rel}"{/if}>{$value.name|escape:'html':'UTF-8'}{if $layered_show_qties}<span> ({$value.nbr})</span>{/if}</a>
												{/if}
											</label>
										</li>
										{/if}
								...
								{else}
									...
									{else}
										<select class="select form-control" {if $filter.filter_show_limit > 1}multiple="multiple" size="{$filter.filter_show_limit}"{/if}>
											...
											{foreach from=$filter.values key=id_value item=value}
											{if $value.nbr || !$hide_0_values}
												<option {if (isset($value.color) && $value.color!='')}style="color: {$value.color}"{/if} id="layered_{$filter.type_lite}{if $id_value || $filter.type == 'quantity'}_{$id_value}{/if}" value="{$id_value}_{$filter.id_key}" {if isset($value.checked) && $value.checked}selected="selected"{/if} {if !$value.nbr}disabled="disabled"{/if}>
													{$value.name|escape:'html':'UTF-8'}{if $layered_show_qties}<span> ({$value.nbr})</span>{/if}
												</option>
											{/if}
											{/foreach}
