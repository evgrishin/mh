    ...
    <!-- Brand and toggle get grouped for better mobile display -->
        <div class="navbar-header">
            <button type="button" class="navbar-toggle btn-outline-inverse" data-toggle="collapse" data-target=".navbar-ex1-collapse">
                <span class="sr-only">{l s='Katalog' mod='leobootstrapmenu'}</span>
                <span class="fa fa-bars">&nbsp;{l s='Katalog' mod='leobootstrapmenu'}</span>
            </button>
        </div>
...
<script type="text/javascript">{literal}
        $(document).ready(function() {
            ...
            {
                $('a.has-category').removeAttr("data-toggle");
            }
        ...
        $(window).resize(function() {
            if( $(window).width() <= 767 ){
                 $('#cavas_menu').addClass("off-canvas-type");
                $('a.has-category').attr("data-toggle","dropdown");
            }
            else {
                $('a.has-category').removeAttr("data-toggle");
